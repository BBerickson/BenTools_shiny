...
CompareRatios <-
  function(list_data,
    ...
    upratio <-
      filter(outlist[[1]], Ratio <= num & Ratio >= 1 / num | Ratio == 0)
    if (n_distinct(upratio$gene) > 0) {
      nick_name3 <-
        paste("Ratio_No_Diff\nn =", n_distinct(upratio$gene))
...
MakePlotOptionFrame <- function(list_data) {
  ...
  if (!is.null(names(list_data_frame))) {
    ...
    for (i in seq_along(list_data_frame$mycol)) {
      if (ldf[i]) {
        list_data_frame$mycol[i] <-
          RgbToHex(my_hex = list_data_frame$mycol[i], tint = log10(i))
      }
    }
...
server <- function(input, output, session) {
  ...
  observeEvent(input$tabs, ignoreInit = TRUE, {
    ...
    toggle(
      ...
      condition = (
        input$tabs == "mainplot" | input$tabs == "clustertool" &
          LIST_DATA$STATE[1] != 0
      )
    )
  ...
  observeEvent(input$colourhex, ignoreInit = TRUE, {
    ...
    if (!is.null(names(LIST_DATA$gene_info))) {
      if (input$colourhex != LIST_DATA$gene_info[[input$selectgenelistoptions]][[input$selectdataoption]]["mycol"]) {
        ...
        reactive_values$Picker_controler <- paste("color", c(sapply(LIST_DATA$gene_info[[1]], "[[", 4)), sep = ":")
      }
    }
  })
